...
def initialize(cfg, state):
    ...
    if cfg.processes.smb.simple.array == []:
        state.smbpar = np.loadtxt(
            state.original_cwd.joinpath("experiment", cfg.processes.smb.simple.file),
            skiprows=1,
            dtype=np.float32,
        )
    else:
...
def update(cfg, state):
    ...
    if (state.t - state.tlast_mb) >= cfg.processes.smb.simple.update_freq:
