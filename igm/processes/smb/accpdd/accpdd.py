...
def update(cfg, state):
    ...
    if (state.t - state.tlast_mb) >= cfg.processes.smb.accpdd.update_freq:
        ...
        accumulation = tf.where(
            state.air_temp <= cfg.processes.smb.accpdd.thr_temp_snow,
            state.precipitation,
            tf.where(
                state.air_temp >= cfg.processes.smb.accpdd.thr_temp_rain,
                0.0,
                state.precipitation
        ...
        for kk in range(state.air_temp.shape[0]):
            ...
            ablation.append(
                tf.where(
                    snow_depth == 0,
                    pos_temp_year[k] * cfg.processes.smb.accpdd.melt_factor_ice,
                    tf.where(
                        pos_temp_year[k] * cfg.processes.smb.accpdd.melt_factor_snow
                        < snow_depth,
                        pos_temp_year[k] * cfg.processes.smb.accpdd.melt_factor_snow,
                        snow_depth
                        + (
                            pos_temp_year[k]
                            - snow_depth / cfg.processes.smb.accpdd.melt_factor_snow
                        )
                        * cfg.processes.smb.accpdd.melt_factor_ice,
                    ),
                )
            )
