...
def get_sliding_weertman_params_args(cfg) -> Dict[str, Any]:

    cfg_numerics = cfg.processes.iceflow.numerics
    cfg_physics = cfg.processes.iceflow.physics

    return {
        "exp_weertman": cfg_physics.sliding.weertman.exponent,
        "regu_weertman": cfg_physics.sliding.weertman.regu_weertman,
        "vert_basis": cfg_numerics.vert_basis,
    }


# ! I dont think this is needed if we have a sliding_law argument - its a bit confusing...
def cost_sliding_weertman(
    U: tf.Tensor,
    V: tf.Tensor,
    fieldin: Dict,
    vert_disc: Tuple,
