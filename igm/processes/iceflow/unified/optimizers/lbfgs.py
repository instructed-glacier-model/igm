...
class OptimizerLBFGS(Optimizer):
    ...
    def _update_memory(
        ...
        # Use a relative threshold to accept pairs where y and s have a non-negligible
        # positive projection. An absolute eps fails on problems where y·s is legitimately
        # small but non-zero (e.g. identity mapping on fine grids), causing all pairs to
        # be rejected and the optimizer to degrade to unscaled gradient descent.
        y_norm = tf.sqrt(tf.reduce_sum(tf.square(y)) + self.eps)
        s_norm = tf.sqrt(tf.reduce_sum(tf.square(s)) + self.eps)
        eps_accept = self.eps * y_norm * s_norm
        return tf.cond(
            dot_ys > eps_accept, update, lambda: (s_flat_mem, y_flat_mem, idx_memory)
        )
