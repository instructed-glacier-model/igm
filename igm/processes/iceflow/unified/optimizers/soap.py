...
class _SOAPLayerState:
    ...
    def __init__(self, shape: list, dtype):
        ...
        # Skip Kronecker preconditioning for oversized matrices (e.g. SpectralConv2D
        # weights). Those layers fall back to plain Adam via the is_matrix=False path.
        self.is_matrix = (
            (len(shape) > 1)
            and (self.m > 1)
            and (self.m <= self.MAX_PRECOND_SIZE)
            and (self.n <= self.MAX_PRECOND_SIZE)
        )
...
class OptimizerSOAP(Optimizer):
    ...
    def minimize_impl(self, inputs: tf.Tensor) -> tf.Tensor:
        first_batch = self.sampler(inputs) if hasattr(self, "sampler") else inputs
        n_batches = first_batch.shape[0]

        if hasattr(self, "sampler") and getattr(self.sampler, "dynamic_augmentation", False):
            static_batches = None
        ...
        for iter in tf.range(self.iter_max):
            ...
            for b in tf.range(n_batches):
                input = tf.expand_dims(batched_inputs[b], 0)
