...
class InterfaceComposite(InterfaceMapping):
    ...
    def _build_sub_mapping(
        cfg: DictConfig, state: State, cfg_sub: DictConfig
    ) -> Mapping:
        ...
        # Merge (not replace) so emulator.network defaults (nb_layers, etc.) are preserved.
        if "network" in sub_dict:
            net = iceflow.setdefault("emulator", {}).setdefault("network", {})
            net.update(sub_dict["network"])
