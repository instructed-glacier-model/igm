...
def select_patches(cfg: DictConfig, state: State, inputs: tf.Tensor):
    ...
    # NOTE — 2026-05-24 B/C separation: the prior `use_credit_scores` branch
    # (Stage 2 of adaptive_unique_strategy) was removed here so that C is
    # fully decoupled from B. C now always uses its own `temporal_variance`
    # scoring, even when `adaptive_training.enabled` is true. B (in
    # `credit_tracker.py`) drives only the retrain cadence — it no longer
    # writes per-window scores back into C. The trade-off (one extra GPU
    # reduce per step) is documented in `credit_tracker.py`'s module
    # docstring.
...
# ===========================================================================
# Credit-tracker observer (B, the temporal lever) was extracted to
# `credit_tracker.py` in the 2026-05-24 B/C separation. It no longer reads
# or writes to this module: see `credit_tracker.py` for the full doc.
# ===========================================================================
