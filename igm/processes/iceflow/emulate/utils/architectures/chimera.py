...
class Chimera(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "dX" not in self.input_names:
            raise ValueError(
                "Chimera requires 'dX' in input_names "
    ...
    def build(self, input_shape) -> None:
        ...
        if input_shape.rank != 4:
            raise ValueError(
                f"Chimera expects input_shape rank 4 [B, H, W, C], "
                f"got {input_shape}"
            )
...
class ChimeraSharedHead(Chimera):
    """
    Shared-spatial-body variant of Chimera.

    This architecture keeps the same explicit physics decomposition as
    ``Chimera`` at the output level:
    ...
    --------------
    nb_layers : int
        Nominal context depth.  As in Chimera, this creates
    ...
    anchor_deformation_at_bed : bool, optional
        Same constraint as Chimera.  Default True.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Chimera so _physics_features can be inherited unchanged.
        # ------------------------------------------------------------------
        self.n_glen = float(self.FIXED_N_GLEN)
        self.rho = float(self.FIXED_RHO)
        self.g = float(self.FIXED_G)
        ...
        if "dX" not in self.input_names:
            raise ValueError(
                "ChimeraSharedHead requires 'dX' in input_names "
                "(grid spacing is read per-pixel from the dX channel)."
            )
        self.idx_dX = self.input_names.index("dX")
    ...
    def build(self, input_shape) -> None:
        ...
        if input_shape.rank != 4:
            raise ValueError(
                f"ChimeraSharedHead expects input_shape rank 4 [B, H, W, C], "
                f"got {input_shape}"
            )
