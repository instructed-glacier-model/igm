...
def initialize(cfg, state):
    ...
    if cfg.processes.clim_station.climate_change_array == []:
        ...
        state.climatepar = np.array(
            cfg.processes.clim_station.climate_change_array[1:]
        ).astype(np.float32)
    ...
    state.reference_precip_ice = (
        cfg.processes.clim_station.reference_precipitation * 1.0989
    )  # conversion from water equivalent to ice equivalent (using ice density of 910 kg m-3)
    state.precip_ref_change = (
        cfg.processes.clim_station.precipitation_lapse_rate / 100
...
def update(cfg, state):
    ...
    if (state.t - state.tlast_clim_update) >= cfg.processes.clim_station.update_freq:
        ...
        if (
            cfg.processes.clim_station.time_dependent_climate
        ...
        state.air_temp.assign_add(
            cfg.processes.clim_station.adiabatic_lapse_rate * delta_usurf_inversed
        )
        ...
        new_precip = tf.maximum(
            new_precip, cfg.processes.clim_station.min_precipitation_allowed
        )
...
def produce_climate_data(cfg, state):
    ...
    precipitation = tf.maximum(
        precipitation, cfg.processes.clim_station.min_precipitation_allowed
    )

    # Create the space-invariant air_temp_sd variable
    air_temp_sd = tf.fill(
        tf.shape(state.thk), cfg.processes.clim_station.air_temperature_stdev
    )
    ...
    if cfg.processes.clim_station.cosine_yearly_cycle_temp:
        ...
        if cfg.processes.clim_station.southern_hemisphere_climate:
            seasonal_cycle = (
                cfg.processes.clim_station.cosine_yearly_cycle_amplitude
    ...
    shift = (
        2 / 12 if not cfg.processes.clim_station.southern_hemisphere_climate else 8 / 12
    )
