...
def initialize(cfg, state):
    ...
    if cfg.processes.climate.station.climate_change_array == []:
        state.climatepar = None
    else:
        state.climatepar = np.array(
            cfg.processes.climate.station.climate_change_array[1:]
        ).astype(np.float32)
...
def update(cfg, state):
    ...
    if (state.t - state.tlast_clim_update) >= cfg.processes.climate.station.update_freq:
        if hasattr(state, "logger"):
            state.logger.info("Construct climate at time : " + str(state.t.numpy()))
        ...
        state.air_temp.assign(
            state.air_temp_ref
        )  # Reset air temperature to initial reference temperature field
        state.precipitation.assign(
            state.precipitation_ref
        )  # Reset precipitation to initial reference precipitation field

        ### If set by user, apply the time-dependent climate changes
        if (
            cfg.processes.climate.station.time_dependent_climate
            and state.climatepar is not None
        ):
            ...
            precip_offset = interp1d_tf(
                state.climatepar[:, 0], state.climatepar[:, 2], state.t
            )
            precip_offset = tf.broadcast_to(
                precip_offset, tf.shape(state.precipitation)
            )
        ...
        # Compute surface elevation difference (how much usurf has changed from usurf_initial)
        delta_usurf = state.usurf - state.usurf_initial
        delta_usurf_inversed = (
            -delta_usurf
        )  # we inverse as surface Temp needs to decrease with increasing elevation offset from usurf_initial
        # Ensure delta_usurf_inversed has the same shape as state.air_temp
        delta_usurf_inversed = tf.broadcast_to(
            delta_usurf_inversed, tf.shape(state.air_temp)
        )
        # Update air temperature using the user-defined adiabatic lapse rate
        state.air_temp.assign_add(
            cfg.processes.climate.station.adiabatic_lapse_rate * delta_usurf_inversed
        )

        ### PRECIP change due to ice surface elevation change
        new_diff_elev_precip = delta_usurf / 100
        # Ensure new_diff_elev_precip has the correct shape
        new_diff_elev_precip = tf.broadcast_to(
            new_diff_elev_precip, tf.shape(state.precipitation)
        )
        ...
        new_precip = tf.maximum(
            new_precip, cfg.processes.climate.station.min_precipitation_allowed
        )
...
def produce_climate_data(cfg, state):
    ...
    air_temp_sd = tf.fill(
        tf.shape(state.thk), cfg.processes.climate.station.air_temperature_stdev
    )
    ...
    if cfg.processes.climate.station.cosine_yearly_cycle_temp:
        months = np.arange(12)
        if cfg.processes.climate.station.southern_hemisphere_climate:
            seasonal_cycle = (
                cfg.processes.climate.station.cosine_yearly_cycle_amplitude
                * np.cos(2 * np.pi * months / 12)
            )
        else:
            seasonal_cycle = (
                cfg.processes.climate.station.cosine_yearly_cycle_amplitude
                * np.cos(2 * np.pi * (months - 6) / 12)
            )
    ...
    if cfg.processes.climate.station.export_climate_ref:
        ds = xr.Dataset(
            {
            ...
            coords={
                ...
                "x": (
                    state.x.numpy()
                    if hasattr(state, "x")
                    else np.arange(air_temp.shape[2])
                ),
                "y": (
                    state.y.numpy()
                    if hasattr(state, "y")
                    else np.arange(air_temp.shape[1])
                ),
            },
        )
        # Save to NetCDF file
    ...
    air_temp_sd = np.roll(
        air_temp_sd.numpy(), int(len(air_temp_sd) * (1 - shift)), axis=0
    )
    precipitation = np.roll(
        precipitation.numpy(), int(len(precipitation) * (1 - shift)), axis=0
    )
