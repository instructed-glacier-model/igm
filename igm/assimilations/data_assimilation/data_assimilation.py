...
def initialize(cfg, state):
    ...
    patience = getattr(cfg.assimilations.data_assimilation.optimization, "patience", 0)
    velsurf_history = []

    # iterate over the optimization process
    for i in range(cfg.assimilations.data_assimilation.optimization.nbitmax+1):

        cost = {}

        if cfg.assimilations.data_assimilation.optimization.method == "ADAM":
            optimize_update(cfg, state, cost, i)
        elif cfg.assimilations.data_assimilation.optimization.method == "L-BFGS":
        ...
        if cfg.assimilations.data_assimilation.optimization.retrain_iceflow_model:
            state.it = i + 1
        ...
        if i % cfg.assimilations.data_assimilation.output.freq == 0:
            if cfg.assimilations.data_assimilation.output.plot2d:
                update_plot_inversion(cfg, state, i)
            if cfg.assimilations.data_assimilation.output.save_iterat_in_ncdf:
        ...
        if patience > 0 and "velsurf" in cost:
            ...
            if i >= cfg.assimilations.data_assimilation.optimization.nbitmin \
    ...
    if not cfg.assimilations.data_assimilation.output.save_result_in_ncdf=="":
