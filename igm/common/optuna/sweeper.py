...
class IGMOptunaSweeper(Sweeper):
    ...
    def _collect_result(self, trial, process, score_file, optimize_cfg):
        ...
        if score_file.exists():
            ...
            logger.warning(f"Trial {trial.number}: no score file, using inf")
        else:
            self.study.tell(trial.number, state=optuna.trial.TrialState.FAIL)
        ...
        for batch_start in range(0, n_trials, n_jobs):
            ...
            for _ in range(batch_count):
                ...
                for cond in optimize_cfg.get("conditional_overrides", []):
                    when = cond.get("when", {})
                    if all(suggested.get(k) == v for k, v in when.items()):
                        for key, val in cond.get("set", {}).items():
                            hydra_val = "null" if val is None else val
                            overrides.append(f"{key}={hydra_val}")
                        for src, targets in cond.get("mirror", {}).items():
                            if src in suggested:
                                for tgt in targets:
                                    overrides.append(f"{tgt}={suggested[src]}")
            ...
            if n_jobs == 1:
                ...
                logger.info(f"Trial {trial.number}: " + " ".join(overrides))
